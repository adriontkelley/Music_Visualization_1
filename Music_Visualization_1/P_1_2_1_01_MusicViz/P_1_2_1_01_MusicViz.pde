...
float heightMult = 2;
  ...
  tileCountX = (int) map(100,0,width,2,100);
  tileCountY = (int) map(100,0,height,2,10);
  ...
  for (int gridY=0; gridY< tileCountY; gridY++) {
    ...
    for (int gridX=0; gridX< tileCountX ; gridX++) { 
      float amount = map(gridX*scoreLow/100,0,random(tileCountX-1),0,1);
      
      if (interpolateShortest) {
        // switch to rgb
        colorMode(RGB,255,255,255,255);
        interCol = lerpColor(col1,col2, amount*scoreLow/100); 
        // switch back
        colorMode(HSB,360,100,100,100);
      } 
      else {
      ...
      rect(posX, random(posY), tileWidth, tileHeight*scoreLow/100); 

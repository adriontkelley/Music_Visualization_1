...
////This sketch requires the Camera3D Processing library
...
float heightMult = 2;
  ...
  strokeWeight(10);
  stroke(scoreLow/1000);

  fill(360*scoreLow/500);
  beginShape(QUADS);
  // left
  ...
  // bottom
  vertex(xMin, yMax, zMin);
  vertex(xMin, yMax*scoreLow/100, zMax);
  vertex(xMax*scoreLow/100, yMax, zMax);
  vertex(xMax, yMax, zMin);

  // top
  vertex(xMin, yMin, zMin*scoreLow/100);
  vertex(xMin, yMin, zMax);
  vertex(xMax, yMin*scoreLow/100, zMax);
  vertex(xMax, yMin, zMin);
  endShape();
